...
| **링크 알아보기** | 쿠팡에서 복사한 주소를 그대로 붙여넣으면 어떤 링크인지 즉시 알려주고, 추적 쿼리(`searchId`, `rank` 등)를 털어냅니다. |
| 정렬 | 가격 낮은순 / 높은순 / **가나다순**(한글 정렬) / 최근 추가순 |
| 가격대 필터 | 소액(3만 원 미만) · 중액(3~10만 원) · 고액(10만 원 이상) |
| 상품 링크 연결 | 아이템의 “상품 보기”를 누르면 실제 판매 페이지로 바로 이동합니다. |
| **2단계 결제 팝업** | “이거 사줄게요” → 상품 페이지가 새 창으로 열리고, 원래 화면은 “결제를 완료해주세요 + 결제 완료 버튼”으로 바뀝니다. |
| 중복 선물 방지 | “이거 사줄게요”를 누르면 30분 동안 그 아이템이 “준비 중”으로 잠깁니다. |
| 감사 이모션 | 결제 완료를 누르면 **축포**(양쪽 아래에서 펑, 잠시 뒤 한 발 더) + “너무 감사합니다!!” 화면이 뜹니다. |
...
### 검색하다 복사한 링크를 그대로 붙여넣어도 됩니다

쿠팡에서 상품을 찾아 주소창을 복사하면 보통 이렇게 생겼습니다.

```
https://www.coupang.com/vp/products/7823456?itemId=2211&vendorItemId=77&searchId=abc123&rank=4&isAddedCart=
```

붙여넣는 즉시 **“🔗 쿠팡 상품 링크예요. 결제가 자동으로 확인됩니다.”** 라고 알려주고,
주소를 이렇게 정리해서 저장합니다. 옵션을 가리키는 `itemId`·`vendorItemId` 는 남기고
검색 흔적(`searchId`, `rank`, `isAddedCart`)만 지웁니다.

```
https://www.coupang.com/vp/products/7823456?itemId=2211&vendorItemId=77
```

쿠팡이 아닌 쇼핑몰 링크도 받습니다. 다만 “이 쇼핑몰은 자동 확인이 안 돼요”라고
알려주고, `utm_*` 같은 추적 쿼리만 털어냅니다.

**파트너스 링크로의 변환은 저장할 때가 아니라 선물할 때 일어납니다.** `subId` 는
선물 한 건마다 달라야 누가 샀는지 되찾을 수 있기 때문에, 원본 주소를 저장해 두었다가
“이거 사줄게요”를 누른 그 사람 몫의 `subId` 로 그때 변환합니다.
...
| `POST` | `/api/link/inspect` | 붙여넣은 링크 알아보기 + 정리 (네트워크 호출 없음) |
